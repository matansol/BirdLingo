...
# Re-using the groups_meta and bird_to_group dictionaries
groups_meta = {
...
with open('assets/birds_data.json', 'r', encoding='utf-8') as f:
    birds_data_list = json.load(f)

# Clear existing groups to start fresh
birds_data_list = [b for b in birds_data_list if not b.get('isGroup')]

groups_dict = {}

for data in birds_data_list:
    # Use image/filename base as the key, e.g. "house_sparrow"
    bird_key = data.get("image", "").strip()
    if bird_key in bird_to_group:
        ...
        if g_id not in groups_dict:
            groups_dict[g_id] = {
                ...
                "category": data.get("category", "Various"),
                "locations": set(data.get("locations", [])),
                "tags": set(data.get("tags", [])),
                "images": [],
                "description": {
                    "en": f"A classification representing {groups_meta[g_id]['en']} species.",
            ...
            # keep difficulty accessible for endless mode filtering
            groups_dict[g_id]["difficulty"] = min(groups_dict[g_id]["difficulty"], data.get("difficulty", 5))
            
            # Combine categories properly
            cur_cat = groups_dict[g_id]["category"]
            new_cat = data.get("category", "")
            if cur_cat != "Various" and cur_cat != new_cat:
                groups_dict[g_id]["category"] = "Various"
...
for data in birds_data_list:
    if not data.get("isGroup", False) and "groupId" in data:
        ...
        if g_id in groups_dict:
            if "images" in data:
                # Add up to 2 images per bird to the group
...
for g_id, g_data in groups_dict.items():
    ...
    g_data["images"] = g_data["images"][:8]
    if len(g_data["images"]) > 0:
        g_data["image"] = g_data["images"][0].split(".")[0] # first image without extension, this field usually represents the base species id
    else:
        g_data["image"] = g_id # fallback
    
    birds_data_list.append(g_data)

with open('assets/birds_data.json', 'w', encoding='utf-8') as f:
    json.dump(birds_data_list, f, ensure_ascii=False, indent=2)

print(f"Added {len(groups_dict)} generic groups to assets/birds_data.json!")
